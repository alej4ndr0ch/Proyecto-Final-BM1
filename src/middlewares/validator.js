...
import { existenteEmail,existenteNameCategorie, existenteNameProduct } from "../helpers/db-validator.js";
...
export const validatorProduct = [
    body('name', 'The name is required').not().isEmpty(),
    body('name').custom(existenteNameProduct),
    body('description', 'The description is required').not().isEmpty(),
    body('precio', 'The price is required').not().isEmpty(),
    body('precio', 'The price must be a positive number').isFloat({ gt: 0 }),
    body('stock', 'The stock is required').not().isEmpty(),
    body('stock', 'The stock must be a non-negative integer').isInt({ min: 0 }),
